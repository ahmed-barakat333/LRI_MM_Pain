...
working_dir <- "~/Documents/transcriptutorial-master" #put whatever is your working directory here
setwd(working_dir)
...
# dorothea <- read.csv("data/omnipath.csv")
...
# omnipath <- read.csv("data/omnipath.csv")
...
names(omnipath_sif) <- c("source","interaction","target")
names(omnipath_sif_2) <- c("source","interaction","target")

omnipath_sif_2$interaction <- omnipath_sif_2$interaction * -1
...
omnipath_sif <- omnipath_sif[!grepl("_",omnipath_sif$source),]
omnipath_sif <- omnipath_sif[!grepl("_",omnipath_sif$target),]
...
result1 = runCARNIVAL(#solverPath="cbc-osx/cbc"
...
result1$weightedSIF <- data.frame(result1$weightedSIF, stringsAsFactors = F)
result1$weightedSIF$Sign <- as.numeric(result1$weightedSIF$Sign)
result1$weightedSIF$Weight <- as.numeric(result1$weightedSIF$Weight)

result1$nodesAttributes <- data.frame(result1$nodesAttributes, stringsAsFactors = F)
result1$nodesAttributes$ZeroAct <- as.numeric(result1$nodesAttributes$ZeroAct)
result1$nodesAttributes$UpAct <- as.numeric(result1$nodesAttributes$UpAct)
result1$nodesAttributes$DownAct <- as.numeric(result1$nodesAttributes$DownAct)
result1$nodesAttributes$AvgAct <- as.numeric(result1$nodesAttributes$AvgAct)

saveRDS(result1,"results/result1.rds")

# visualization
visNet = carnival_visNet(evis = result1$weightedSIF,
                         nvis = result1$nodesAttributes)
