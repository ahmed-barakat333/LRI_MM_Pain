...
mm_cells_order <- c("Plasma Cells","Bone Marrow Stromal Cells", "Endothelial Cell", "Osteocyte","Osteogenic Precursor Cell",
                    "Adipocyte", "Natural Killer Cell", "Neutrophil", "Macrophage",
                    "Regulatory T cell", "Hematopoietic Stem Cell","Whole Bone Marrow")
...
colnames(ec_pathways)[1] <- "Endothelial Cell"
...
colnames(ocy_pathways)[1] <- "Osteocyte"
...
colnames(opc_pathways)[1] <- "Osteogenic Precursor Cell"
...
colnames(adcy_pathways)[1] <- "Adipocyte"
...
colnames(nkc_pathways)[1] <- "Natural Killer Cell"
...
colnames(neut_pathways)[1] <- "Neutrophil"
...
colnames(mp_pathways)[1] <- "Macrophage"
...
colnames(treg_pathways)[1] <- "Regulatory T cell"
...
colnames(hsc_pathways)[1] <- "Hematopoietic Stem Cell"
...
### test overlap
pathways <- list("Plasma Cells" = pc_pathways$`Plasma Cells`,"Bone Marrow Stromal Cells" =bmsc_pathways$`Bone Marrow Stromal Cells`,"Endothelial Cell"  =ec_pathways$`Endothelial Cell`, 
                 "Osteocyte"=ocy_pathways$Osteocyte, "Osteogenic Precursor Cell"=opc_pathways$`Osteogenic Precursor Cell`,"Adipocyte"=adcy_pathways$Adipocyte, "Natural Killer Cell"=nkc_pathways$`Natural Killer Cell`, "Neutrophil"=neut_pathways$Neutrophil, 
                 "Macrophage"=mp_pathways$Macrophage,"Regulatory T cell"=treg_pathways$`Regulatory T cell`,"Hematopoietic Stem Cell"=hsc_pathways$`Hematopoietic Stem Cell`,
...
tiff("results/pathways_dot_plot.tiff", units="in", width=8, height=7, res=300)
...
colnames(ec_ligands)[1] <- "Endothelial Cell"
...
colnames(ocy_ligands)[1] <- "Osteocyte"
...
### test overlap
ligands <- list("Plasma Cells" = pc_ligands$`Plasma Cells`,"Bone Marrow Stromal Cells" =bmsc_ligands$`Bone Marrow Stromal Cells`,"Endothelial Cell"  =ec_ligands$`Endothelial Cell`, 
                "Osteocyte"=ocy_ligands$Osteocyte,"Natural Killer Cell"=nkc_ligands$`Natural Killer Cell`)
...
colnames(ec_kinases)[1] <- "Endothelial Cell"
...
colnames(ocy_kinases)[1] <- "Osteocyte"
...
### test overlap
kinases <- list("Plasma Cells" = pc_kinases$`Plasma Cells`,"Bone Marrow Stromal Cells" =bmsc_kinases$`Bone Marrow Stromal Cells`,"Endothelial Cell"  =ec_kinases$`Endothelial Cell`, 
                "Osteocyte"=ocy_kinases$Osteocyte,"Natural Killer Cell"=nkc_kinases$`Natural Killer Cell`)
...
colnames(ec_kinases)[2] <- "Endothelial Cell"
...
colnames(ocy_kinases)[2] <- "Osteocyte"
...
colnames(adcy_kinases)[2] <- "Adipocyte"
...
colnames(mp_kinases)[2] <- "Macrophage"
...
colnames(treg_kinases)[2] <- "Regulatory T cell"
...
kinases <- melt(setDT(kinases), id.vars = c("Protein.Kinase"), variable.name = "Cell")
colnames(kinases)[3] <- "log10_p_value"

tiff("results/cells_kinases_dot_plot.tiff", units="in", width=8, height=15, res=100)

ggplot(kinases, aes(x= Cell, y = Protein.Kinase, size = log10_p_value)) +
...
colnames(ec_tf)[1] <- "Endothelial Cell"
ec_tf <- ec_tf[1]
...
### test overlap
tf <- list("Plasma Cells" = pc_tf$`Plasma Cells`,"Bone Marrow Stromal Cells" =bmsc_tf$`Bone Marrow Stromal Cells`,"Endothelial Cell"  =ec_tf$`Endothelial Cell`, 
           "Osteocyte"=ocy_tf$Osteocyte,"Natural Killer Cell"=nkc_tf$`Natural Killer Cell`)
...
colnames(ec_tf)[2] <- "Endothelial Cell"
...
colnames(treg_tf)[2] <- "Regulatory T cell"
...
tf <- melt(setDT(tf), id.vars = c("Transcription.Factor"), variable.name = "Cell")
colnames(tf)[3] <- "log10_p_value"
tf[tf==0] <- NA
tf<-tf[complete.cases(tf),]

tiff("results/cells_tf_dot_plot.tiff", units="in", width=8, height=15, res=100)

ggplot(tf, aes(x= Cell, y = Transcription.Factor, size = log10_p_value)) +
  geom_point(colour="grey") +
  scale_size_continuous(range = c(0.5,6)) +
  theme_classic() +
  xlab("Multiple Myeloma Cells") +
  ...
  theme(plot.title = element_text(size = 15)) +
  ggtitle("Enriched Transcription Factors") +
  theme(axis.title = element_text(size=12)) + 
  theme(axis.text.x = element_text( color="#000000", size=12, angle=90, hjust = 0.95, vjust = 0.4),
        axis.text.y = element_text( color="#000000", size=12)) +
  theme( axis.line = element_line(colour = "#000000", linewidth = 0.5, linetype = "solid")) +
...
proteomic_data <- load("data/proteomes/data_proteomes_transcriptomes.RData")
